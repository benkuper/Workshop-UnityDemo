...
public class UIMaster : MonoBehaviour
{
    ...
    public void CreateUI(Controllable newControllable)
    {
        ...
        {
            var propertyType = getInfoType(property.Value);
    ...
    private void CreateDropDown(Transform parent, Controllable target, MemberInfo listProperty, MemberInfo activeElement)
    {
        var newDropdown = Instantiate(DropdownPrefab);
        newDropdown.transform.SetParent(parent);

        //TODO remove string
        var listInObject = (List<string>)target.getInfoValue(listProperty);;
        var activeElementInObject = target.getInfoValue(activeElement).ToString();
        ...
        {
            var associatedList = (List<string>)target.getInfoValue(listProperty);
            string activeItem = associatedList[value];

            List<object> objParams = new List<object> { activeItem };
            target.setValueProp(activeElement, objParams);
        });
        ...
        target.controllableValueChanged += (name) =>
        {
            ...
            activeElementInObject = target.getInfoValue(activeElement).ToString();
            listInObject = (List<string>)target.getInfoValue(listProperty);
    ...
    private void CreateSlider(Transform parent, Controllable target, MemberInfo property, RangeAttribute rangeAttribut, bool isInteractible, bool isFloat = true)
    {
        //Debug.Log("Range : " + rangeAttribut.min + ";" + rangeAttribut.max);
        String type = getInfoType(property).ToString();
        ...
        inputComponent.onEndEdit.AddListener((value) =>
        {
            ...
            target.setValueProp(property, list);
        });
        ...
        // Debug.Log("Value of " + property.Name + " is " + float.Parse(target.getInfoValue(property).ToString()));
        ...
        sliderComponent.onValueChanged.AddListener((value) =>
        {
            ...
            list.Add(value);
            target.setValueProp(property, list);
            inputComponent.text = target.getInfoValue(property).ToString();
        });
        target.controllableValueChanged += (name) =>
        {
            ...
            if (name == property.Name)
            {
                if (isFloat)
                {
                    sliderComponent.value = (float)target.getInfoValue(target.getPropFieldInfoForAddress(name));
                    var str = "" + target.getInfoValue(property);
                    str = str.Replace(",", ".");
                    inputComponent.text = "" + str;
                }
                else
                {
                    sliderComponent.value = (int)target.getInfoValue(target.getPropFieldInfoForAddress(name));
                    inputComponent.text = sliderComponent.value.ToString();
                }
            }
        };
        if (isFloat)
            sliderComponent.value = float.Parse(target.getInfoValue(property).ToString());
        else
            sliderComponent.value = (int)target.getInfoValue(property);
        newSlider.GetComponent<RectTransform>().localScale = new Vector3(1.0f, 1.0f, 1.0f);
    }

    private void CreateInput(Transform parent, Controllable target, MemberInfo property, bool isInteractible)
    {
        ...
        newInput.GetComponentInChildren<InputField>().text = target.getInfoValue(property).ToString();
        newInput.GetComponentInChildren<InputField>().onEndEdit.AddListener((value) =>
        {
            var list = new List<object>();
            var propertyType = getInfoType(property);
            if (showDebug)
            {
        ...
        newInput.transform.GetChild(0).Find("Placeholder").gameObject.GetComponent<Text>().text = target.getInfoValue(target.getPropFieldInfoForAddress(property.Name)).ToString();
        newInput.GetComponent<RectTransform>().localScale = new Vector3(1.0f, 1.0f, 1.0f);
    }

    private void CreateCheckbox(Transform parent, Controllable target, MemberInfo property, bool isInteractible)
    {
        ...
        newCheckbox.GetComponent<Toggle>().isOn = (bool) target.getInfoValue(property);
        ...
        target.controllableValueChanged += (name) =>
        {
            ...
            if (name == property.Name) {
                var newValue = (bool)target.getInfoValue(target.getPropFieldInfoForAddress(name));;
